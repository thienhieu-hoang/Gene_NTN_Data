...
bs_velocities_tensor = tf.constant([[v_sat_ENU]], dtype=tf.float32)  # Shape [1, 1, 3]
in_state = tf.constant([[False]], dtype=tf.bool)                     # Shape [1, 1]

# Apply custom ENU geometry to the topology
topology_data = (ut_loc_tensor, bs_loc_tensor, ut_orientations, bs_orientations,
                 ut_velocities_tensor, bs_velocities_tensor, in_state)
channel_model.set_topology(*topology_data, los=True)

# Generate channel path coefficients and delays (with same seed for alignment)
channel_seed = 42
...
# -------------------------------------------------------------------------
# Part 1: Full/Uncompensated Channel Generation (full Doppler)
# -------------------------------------------------------------------------
...
tf.random.set_seed(channel_seed)
h_freq_full = ofdm_channel() # shape [1, 1, 1, 1, 2, 14, 256]
h_eff_full = remove_nulled(h_freq_full)   # shape [1, 1, 1, 1, 2, 14, 132]
h_eff_siso_full = h_eff_full[:, 0, 0, 0, 0, :, :].numpy() # shape [1, 14, 132]

# -------------------------------------------------------------------------
# Part 2: Precompensated Channel Generation (precompensated Doppler)
# -------------------------------------------------------------------------
...
tf.random.set_seed(channel_seed)
h_freq_comp = ofdm_channel() # shape [1, 1, 1, 1, 2, 14, 256]
h_eff_comp = remove_nulled(h_freq_comp)   # shape [1, 1, 1, 1, 2, 14, 132]
h_eff_siso_comp = h_eff_comp[:, 0, 0, 0, 0, :, :].numpy() # shape [1, 14, 132]

# -------------------------------------------------------------------------
# Part 3: Signal Transmission & Channel Estimation (LS + Interpolation)
# -------------------------------------------------------------------------
# Generate QPSK tx_grid (same for both)
np.random.seed(channel_seed)
tx_grid = np.random.choice(qpsk_symbols, size=[batch_size, 14, 132])

# Generate common noise pattern
np.random.seed(channel_seed + 10)
noise_pattern = (np.random.randn(*h_eff_siso_full.shape) + 1j * np.random.randn(*h_eff_siso_full.shape))

# A. Pass through Full Doppler Channel & Estimate
rx_signal_clean_full = tx_grid * h_eff_siso_full
sig_power_full = np.mean(np.abs(rx_signal_clean_full) ** 2)
...
h_LS_full = rx_grid_full / tx_grid # shape [1, 14, 132]
pilot_symbols, pilot_subcarriers = np.where(pilot_mask)
h_LS_pilots_full = h_LS_full[:, pilot_symbols, pilot_subcarriers]

h_interp_full_batch = []
for b in range(batch_size):
    h_interp_b = interpolate_channel(rx_grid_full[b], tx_grid[b], pilot_mask)
    h_interp_full_batch.append(h_interp_b)
h_interp_full_batch = np.stack(h_interp_full_batch, axis=0) # [1, 14, 132]

# B. Pass through Precompensated Channel & Estimate
rx_signal_clean_comp = tx_grid * h_eff_siso_comp
sig_power_comp = np.mean(np.abs(rx_signal_clean_comp) ** 2)
...
h_LS_comp = rx_grid_comp / tx_grid # shape [1, 14, 132]
h_LS_pilots_comp = h_LS_comp[:, pilot_symbols, pilot_subcarriers]

h_interp_comp_batch = []
for b in range(batch_size):
    h_interp_b = interpolate_channel(rx_grid_comp[b], tx_grid[b], pilot_mask)
