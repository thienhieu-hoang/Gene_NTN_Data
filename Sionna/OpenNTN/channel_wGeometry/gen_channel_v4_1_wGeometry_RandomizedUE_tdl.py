...
from OpenNTN import TDL, DenseUrban_modify, Urban_modify, SubUrban_modify
...
# Target delay spread: custom value or 3GPP TR 38.811 standard default (100 ns)
delay_spread_s = (delay_spread_ns_custom * 1e-9) if delay_spread_ns_custom is not None else 100e-9

# Initialize OpenNTN TDL channel models for Full Doppler and Precompensated Doppler
tdl_full = TDL(model=tdl_model_name,
               delay_spread=delay_spread_s,
               carrier_frequency=carrier_frequency,
               min_speed=v_min,
               max_speed=v_max,
               basestation_height=satellite_height,
               elevation_angle=elevation_angle_nom,
               doppler_mode='full')

tdl_comp = TDL(model=tdl_model_name,
               delay_spread=delay_spread_s,
               carrier_frequency=carrier_frequency,
               min_speed=v_min,
               max_speed=v_max,
               basestation_height=satellite_height,
               elevation_angle=elevation_angle_nom,
               doppler_mode='precompensated')

ofdm_channel_full = GenerateOFDMChannel(tdl_full, resource_grid=rg)
ofdm_channel_comp = GenerateOFDMChannel(tdl_comp, resource_grid=rg)
...
for b in range(num_batches):
    ...
    path_coefficients_full, path_delays_full = tdl_full(current_batch_size, num_time_steps, sampling_frequency)
    ...
    if b == 0:
        ...
        try:
            ...
            ax1.set_title(f"Precompensated Doppler: Real Part of Channel 0 ({tdl_model_name})\nElevation: {elevation_angles_all[0]:.2f}°")
            ax1.set_xlabel("OFDM Symbol Index")
            ax1.set_ylabel("Subcarrier Index")
            fig1.colorbar(im1, ax=ax1)
            plt.tight_layout()
            channel_real_comp_filename = os.path.join(output_dir, f"channel_real_comp_{tdl_model_name}.pdf")
...
# Save .mat file
mat_filename = os.path.join(output_dir, f"channel_{tdl_model_name}_randomizedUE.mat")
mat_data = {
...
- **TDL Model Profile**: {tdl_model_name} (3GPP TR 38.811 NTN Profile)
