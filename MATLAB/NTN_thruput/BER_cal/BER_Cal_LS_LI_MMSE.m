...
numUE = 100;                    % Number of UEs (samples) for simulation
...
%% 4b. Load Trained CNN Models for LS and LI (single_source_trained_model)
source_model_folder_name = 'DUR100ns_2p18G_600km_70deg_r15km_20to30mps';
source_model_dir = fullfile(script_dir, 'single_source_trained_model', source_model_folder_name);

snr_str = num2str(SNRdB);
mat_ls_path = fullfile(source_model_dir, ['LS_', snr_str], 'results', 'best_net.mat');
mat_li_path = fullfile(source_model_dir, ['LI_', snr_str], 'results', 'best_net.mat');

% Priority search for best_net.mat files
if ~exist(mat_ls_path, 'file')
    ls_list = dir(fullfile(source_model_dir, '**', '*ls*', 'best_net.mat'));
    if isempty(ls_list)
        ls_list = dir(fullfile(script_dir, 'single_source_trained_model', '**', '*ls*', 'best_net.mat'));
    end
    if ~isempty(ls_list)
        mat_ls_path = fullfile(ls_list(1).folder, ls_list(1).name);
    end
end

if ~exist(mat_li_path, 'file')
    li_list = dir(fullfile(source_model_dir, '**', '*li*', 'best_net.mat'));
    if isempty(li_list)
        li_list = dir(fullfile(script_dir, 'single_source_trained_model', '**', '*li*', 'best_net.mat'));
    end
    if ~isempty(li_list)
        mat_li_path = fullfile(li_list(1).folder, li_list(1).name);
    end
end

net_ls = load_mat_model_helper(mat_ls_path);
net_li = load_mat_model_helper(mat_li_path);
...
fprintf('Running transmission, channel estimation (LS+LI, LS+MMSE, H_ls_cnn, H_li_cnn), and BER evaluation at SNR = %d dB...\n', SNRdB);
...
for idxUE = 1:numUE
    ...
    % ---------------------------------------------------------------------
    % Approach A: LS + Linear Interpolation (LS+LI)
    % ---------------------------------------------------------------------
    ...
    % ---------------------------------------------------------------------
    y_p = rxGrid(dmrsIndices) ./ dmrsSymbols;
    ...
    % ---------------------------------------------------------------------
    % Approach C: CNN Model with Sparse LS Input (H_ls_cnn)
    % ---------------------------------------------------------------------
    % Construct sparse 2D grid: non-zero ONLY at pilot locations, zero elsewhere
    ...
    if ~isempty(net_ls)
        ...
        H_est_ls_cnn = H_ls_grid; % Fallback if model not loaded
    end
    ...
    if ~isempty(net_li)
        ...
        H_est_li_cnn = H_est_LI; % Fallback if model not loaded
    end
    ...
    % Store channel estimates for Sample 1 for PDF heatmap visualizations
    if idxUE == 1
...
% Construct Source and Target System tags
...
result_folder   = fullfile(domain_dir, snr_folder_name);
...
fprintf('3. H_ls_cnn (CNN with Sparse LS Input):\n');
fprintf('   - Mean BER : %.6f\n', mean(ber_ls_cnn));
fprintf('   - Mean NMSE: %.2f dB (%.6f)\n', 10*log10(mean(nmse_ls_cnn)), mean(nmse_ls_cnn));
fprintf('4. H_li_cnn (CNN with Linear Interpolation Input):\n');
fprintf('   - Mean BER : %.6f\n', mean(ber_li_cnn));
...
save(mat_file, ...
    'ber_LI', 'ber_MMSE', 'ber_ls_cnn', 'ber_li_cnn', ...
    'nmse_LI', 'nmse_MMSE', 'nmse_ls_cnn', 'nmse_li_cnn', ...
    'SNRdB', 'numUE', 'simParameters', 'carrier', 'pdsch');

%% Plot & Save Channel Grid Vector PDF Heatmaps for Sample 1
sample_idx = 1;

% Save complex 2D channel grids for Sample 1 into a MAT file for custom plotting/scaling
...
save(sample_mat_file, ...
    'H_perfect_ori_sample1', 'H_perfect_eff_sample1', ...
    'H_ls_sample1', 'H_li_sample1', ...
    'H_ls_cnn_sample1', 'H_li_cnn_sample1', ...
    'SNRdB', 'simParameters', 'carrier');
...
fid = fopen(md_path, 'w');
if fid ~= -1
    fprintf(fid, '# NTN Channel Estimation & BER Simulation Results\n\n');
    fprintf(fid, '```text\n');
    fprintf(fid, '=========================================================\n');
    ...
    fprintf(fid, '   - Mean BER : %.6f\n', mean(ber_MMSE));
    fprintf(fid, '   - Mean NMSE: %.2f dB (%.6f)\n', 10*log10(mean(nmse_MMSE)), mean(nmse_MMSE));
    fprintf(fid, '3. H_ls_cnn (CNN with Sparse LS Pilot Input):\n');
    fprintf(fid, '   - Mean BER : %.6f\n', mean(ber_ls_cnn));
    fprintf(fid, '   - Mean NMSE: %.2f dB (%.6f)\n', 10*log10(mean(nmse_ls_cnn)), mean(nmse_ls_cnn));
    fprintf(fid, '4. H_li_cnn (CNN with Linear Interpolation Input):\n');
    fprintf(fid, '   - Mean BER : %.6f\n', mean(ber_li_cnn));
    fprintf(fid, '   - Mean NMSE: %.2f dB (%.6f)\n', 10*log10(mean(nmse_li_cnn)), mean(nmse_li_cnn));
    fprintf(fid, '=========================================================\n');
    fprintf(fid, '```\n\n');
    ...
    fprintf(fid, '## Channel Grid Visualizations (Sample 1)\n');
    fprintf(fid, '- **Perfect Original Channel Vector PDF:** [`H_perfect_ori_sample1.pdf`](file:///%s)\n', strrep(pdf_ori, '\', '/'));
    fprintf(fid, '- **Effective Compensated Channel Vector PDF:** [`H_perfect_eff_sample1.pdf`](file:///%s)\n', strrep(pdf_eff, '\', '/'));
    fprintf(fid, '- **Sparse LS Estimated Channel Vector PDF:** [`H_ls_sample1.pdf`](file:///%s)\n', strrep(pdf_ls, '\', '/'));
    fprintf(fid, '- **Linear Interpolation Channel Vector PDF:** [`H_li_sample1.pdf`](file:///%s)\n', strrep(pdf_li, '\', '/'));
    fprintf(fid, '- **H_ls_cnn Model Estimated Channel PDF:** [`H_ls_cnn_sample1.pdf`](file:///%s)\n', strrep(pdf_ls_cnn, '\', '/'));
    fprintf(fid, '- **H_li_cnn Model Estimated Channel PDF:** [`H_li_cnn_sample1.pdf`](file:///%s)\n', strrep(pdf_li_cnn, '\', '/'));
    fclose(fid);
end
...
%% =========================================================================
%% HELPER FUNCTIONS FOR GEOMETRY & CNN INFERENCE
%% =========================================================================
function H_pred = predict_cnn_channel(net, H_in)
    nSubc = size(H_in, 1);
    nSymb = size(H_in, 2);
    
    x_raw = zeros(nSubc, nSymb, 2, 'single');
    x_raw(:, :, 1) = single(real(H_in));
    x_raw(:, :, 2) = single(imag(H_in));
    ...
    x_scaled = 2 * (x_raw - min_val) / range_val - 1;
    
    try
        out = predict(net, x_scaled);
        if ndims(out) == 4
            out = squeeze(out);
        elseif ndims(out) == 2 && numel(out) == nSubc * nSymb * 2
            out = reshape(out, [nSubc, nSymb, 2]);
        end
        
        if max(abs(out(:))) <= 2.0
            x_unscaled = (out + 1) / 2 * range_val + min_val;
        else
            x_unscaled = out;
        end
        H_pred = double(x_unscaled(:, :, 1) + 1j * x_unscaled(:, :, 2));
    catch
        H_pred = double(H_in);
    end
end
...
function net = load_mat_model_helper(mat_path)
    net = [];
    if ~exist(mat_path, 'file')
        fprintf('MAT model file not found at: %s\n', mat_path);
        return;
    end
    
    fprintf('Loading MAT model: %s\n', mat_path);
    try
        data = load(mat_path);
        if isfield(data, 'net')
            net = data.net;
            fprintf('  -> Successfully loaded network object from MAT file.\n');
        elseif isfield(data, 'onnx_weights')
            net = data.onnx_weights;
            fprintf('  -> Successfully loaded weight matrices from MAT file.\n');
        else
            fprintf('  -> Warning: Neither "net" nor "onnx_weights" field found in MAT file.\n');
        end
    catch ME
        warning('Failed to load MAT model "%s": %s', mat_path, ME.message);
    end
end
