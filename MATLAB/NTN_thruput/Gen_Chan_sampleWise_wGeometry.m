...
% simParameters.SatelliteAltitude = 600000; % 1000 km altitude
...
% Orbit Alignment (Closest approach / Zenith peak at t=0)
...
% Compute Satellite State at Snapshot Time (Set Elevation Angle)
...
% Compute Satellite State at Overhead Zenith Position (t = 0 s, ~90 deg Elevation)
[r_sat_ECEF_t0, v_sat_ECEF_t0] = get_satellite_state_ecef_local( ...
...
bs_loc_ENU_t0 = R_ECEF2ENU * (r_sat_ECEF_t0 - r_ue_ECEF_0);
v_sat_ENU_t0  = R_ECEF2ENU * v_sat_ECEF_t0;
...
for i = 1:numUE
    ...
    % Geometry calculations at set elevation angle (LOS vector, Slant Range, Elevation Angle)
...
%% 6. Output Folder Definition (Using 'p' instead of '.' for decimal values)
profile_str = erase(channel.DelayProfile, 'NTN-TDL-');
ds_str = num2str(channel.DelaySpread * 1e9);
ds_str = strrep(ds_str, '.', 'p');
...
for snr_idx = 1:length(SNRdB)
    ...
    %% 9. Save OFDM Channel Magnitude PDF Heatmaps (First UE Sample)
    ...
    %% 10. PLOT & SAVE GEOMETRY VISUALIZATION FIGURES
    % (1) Overhead Zenith Position Geometry (t = 0 s, ~90 deg elevation) for 1 UE
    % (2) Orbital Pass Timeline & Doppler/Elevation S-Curves
    
    % --- (1) Geometry at Overhead Zenith (t = 0 s) for 1 UE ---
    plot_3d_ecef_geometry(r_sat_ECEF_t0, v_sat_ECEF_t0, r_ue_ECEF_0, r_ue_ECEF_all(:, 1), ...
        lambda_UE, phi_UE, a_wgs84, r_beam, omega_s, u_mid, Omega_RAAN, inclination, r_orbit, v_sat_orbit, omega_E, ...
        '3D Geometry Snapshot (ECEF Coordinates)\nOverhead Zenith Peak (t = 0 s, ~90° Elevation)', ...
        fullfile(save_folder, 'geometry_ecef_t0_overhead.pdf'));
        
    plot_3d_enu_geometry(bs_loc_ENU_t0, v_sat_ENU_t0, ut_loc_ENU_all(:, 1), r_beam, ...
        '3D Geometry (Local Tangent Plane ENU)\nOverhead Zenith Peak (t = 0 s, ~90° Elevation)', ...
        fullfile(save_folder, 'geometry_enu_t0_overhead.pdf'));

    % --- (2) Orbital Pass Timeline & Doppler / Elevation S-Curves ---
    plot_doppler_elevation_timeline(t_snapshot, simParameters.ElevationAngle, ...
        omega_s, u_mid, Omega_RAAN, inclination, r_orbit, v_sat_orbit, omega_E, ...
        a_wgs84, r_ue_ECEF_0, lambda, simParameters.CarrierFrequency, c, ...
        fullfile(save_folder, 'geometry_doppler_elevation_pass.pdf'));

end

%% =========================================================================
%% HELPER FUNCTIONS FOR GEOMETRY CALCULATIONS AND PLOTTING
%% =========================================================================
...
function plot_3d_ecef_geometry(r_sat_ECEF, v_sat_ECEF, r_ue_ECEF_0, r_ue_ECEF_single, ...
    lambda_UE, phi_UE, a_wgs84, r_beam, omega_s, u_mid, Omega_RAAN, inclination, r_orbit, v_sat_orbit, omega_E, ...
    title_str, pdf_filename)

    % Compute satellite orbit path snippet (-150s to +150s)
    t_path = linspace(-150, 150, 100);
    sat_path_ECEF = zeros(3, length(t_path));
    for tp_idx = 1:length(t_path)
        [sat_path_ECEF(:, tp_idx), ~] = get_satellite_state_ecef_local( ...
            t_path(tp_idx), omega_s, u_mid, Omega_RAAN, inclination, r_orbit, v_sat_orbit, omega_E);
    end

    % Rotation matrix ENU -> ECEF at Beam Center
    R_ENU2ECEF = [ ...
        -sin(lambda_UE), -sin(phi_UE)*cos(lambda_UE), cos(phi_UE)*cos(lambda_UE); ...
         cos(lambda_UE), -sin(phi_UE)*sin(lambda_UE), cos(phi_UE)*sin(lambda_UE); ...
         0,               cos(phi_UE),                sin(phi_UE) ...
    ...
    % Generate Beam Footprint Circle (15 km radius) projected on Earth Surface
    theta_circle = linspace(0, 2*pi, 100);
    circle_ENU = [r_beam * cos(theta_circle); r_beam * sin(theta_circle); zeros(1, 100)];
    circle_ECEF = r_ue_ECEF_0 + R_ENU2ECEF * circle_ENU;
    norm_r0 = norm(r_ue_ECEF_0);
    for k = 1:100
        circle_ECEF(:, k) = circle_ECEF(:, k) * (norm_r0 / norm(circle_ECEF(:, k)));
    end

    fig = figure('Visible', 'off', 'Color', 'w', 'Position', [100, 100, 950, 800]);
    ax = axes('Parent', fig, 'Color', 'w');
    grid(ax, 'on'); hold(ax, 'on'); 

    % Optimized 3D Viewpoint to clearly separate Satellite, UE, and Beam Center
    view(ax, [-45, 20]);
    axis(ax, 'equal');

    % Local WGS-84 Earth Patch Surface
    [XS, YS, ZS] = get_local_earth_patch_local(lambda_UE, phi_UE, a_wgs84, 10.0, 30);
    surf(ax, XS, YS, ZS, 'FaceColor', [0.85, 0.90, 0.95], 'EdgeColor', [0.70, 0.75, 0.80], 'FaceAlpha', 0.5, 'EdgeAlpha', 0.25);

    % LEO Orbit Path & Beam Footprint Circle
    plot3(ax, sat_path_ECEF(1,:), sat_path_ECEF(2,:), sat_path_ECEF(3,:), '--', 'Color', [0.85, 0.60, 0.05], 'LineWidth', 2.0, 'DisplayName', 'LEO Orbit Path');
    plot3(ax, circle_ECEF(1,:), circle_ECEF(2,:), circle_ECEF(3,:), 'Color', [0.5, 0.0, 0.5], 'LineWidth', 2.5, 'DisplayName', 'Beam Footprint (15 km)');

    % Beam Center Marker
    scatter3(ax, r_ue_ECEF_0(1), r_ue_ECEF_0(2), r_ue_ECEF_0(3), 140, [0.5, 0.0, 0.5], 'x', 'LineWidth', 3.0, 'DisplayName', 'Beam Center');

    % Satellite Marker
    scatter3(ax, r_sat_ECEF(1), r_sat_ECEF(2), r_sat_ECEF(3), 180, [0.85, 0.60, 0.05], 'filled', 'MarkerEdgeColor', 'w', 'DisplayName', 'LEO Satellite');

    % Single UE Position Marker
    scatter3(ax, r_ue_ECEF_single(1), r_ue_ECEF_single(2), r_ue_ECEF_single(3), 120, [0.85, 0.20, 0.20], 'filled', 'MarkerEdgeColor', 'w', 'DisplayName', 'User Equipment (UE 1)');

    % Line of Sight (LOS) to UE
    plot3(ax, [r_ue_ECEF_single(1), r_sat_ECEF(1)], [r_ue_ECEF_single(2), r_sat_ECEF(2)], [r_ue_ECEF_single(3), r_sat_ECEF(3)], '--', 'Color', [0.85, 0.20, 0.20], 'LineWidth', 1.5, 'DisplayName', 'Line of Sight (UE 1)');
    
    % Boresight Line from Satellite to Beam Center
    plot3(ax, [r_ue_ECEF_0(1), r_sat_ECEF(1)], [r_ue_ECEF_0(2), r_sat_ECEF(2)], [r_ue_ECEF_0(3), r_sat_ECEF(3)], ':', 'Color', [0.0, 0.55, 0.65], 'LineWidth', 1.8, 'DisplayName', 'Beam Boresight');

    % Satellite Velocity Vector Quiver
    vel_scale_sat = 1.0e5 / 7500 * 4;
    v_sat_scaled = v_sat_ECEF * vel_scale_sat;
    quiver3(ax, r_sat_ECEF(1), r_sat_ECEF(2), r_sat_ECEF(3), v_sat_scaled(1), v_sat_scaled(2), v_sat_scaled(3), 0, 'Color', [0.10, 0.65, 0.35], 'LineWidth', 2.5, 'MaxHeadSize', 0.4, 'DisplayName', 'Sat Velocity (Scaled)');

    % Set tight 3D box centered between ground level and satellite altitude
    mid_pt = (r_ue_ECEF_0 + r_sat_ECEF) / 2.0;
    dist_sat_bc = norm(r_sat_ECEF - r_ue_ECEF_0);
    half_span = dist_sat_bc * 0.70;
    
    xlim(ax, [mid_pt(1) - half_span, mid_pt(1) + half_span]);
    ylim(ax, [mid_pt(2) - half_span, mid_pt(2) + half_span]);
    zlim(ax, [mid_pt(3) - half_span, mid_pt(3) + half_span]);

    xlabel(ax, 'ECEF X (m)', 'FontSize', 12);
    ylabel(ax, 'ECEF Y (m)', 'FontSize', 12);
    zlabel(ax, 'ECEF Z (m)', 'FontSize', 12);
    title(ax, sprintf(title_str), 'FontSize', 14, 'Color', 'k');
    legend(ax, 'Location', 'northeast', 'Color', 'w');
...
function plot_3d_enu_geometry(bs_loc_ENU, v_sat_ENU, ut_loc_ENU_single, r_beam, title_str, pdf_filename)
    fig = figure('Visible', 'off', 'Color', 'w', 'Position', [100, 100, 850, 700]);
    ax = axes('Parent', fig, 'Color', 'w');
    grid(ax, 'on'); hold(ax, 'on'); view(ax, 3);

    % Beam footprint circle (15 km)
    theta_circle = linspace(0, 2*pi, 100);
    plot3(ax, r_beam * cos(theta_circle), r_beam * sin(theta_circle), zeros(1, 100), 'Color', [0.5, 0.0, 0.5], 'LineWidth', 2, 'DisplayName', 'Beam Footprint (15 km)');
    scatter3(ax, 0, 0, 0, 120, [0.5, 0.0, 0.5], 'x', 'LineWidth', 2.5, 'DisplayName', 'Beam Center');

    % Satellite Marker
    scatter3(ax, bs_loc_ENU(1), bs_loc_ENU(2), bs_loc_ENU(3), 180, [0.0, 0.45, 0.74], 'filled', 'MarkerEdgeColor', 'w', 'DisplayName', 'LEO Satellite');

    % Single UE Position
    scatter3(ax, ut_loc_ENU_single(1), ut_loc_ENU_single(2), ut_loc_ENU_single(3), 120, [0.85, 0.20, 0.20], 'filled', 'MarkerEdgeColor', 'w', 'DisplayName', 'User Equipment (UE 1)');

    % Line of Sight to UE 1
    plot3(ax, [ut_loc_ENU_single(1), bs_loc_ENU(1)], [ut_loc_ENU_single(2), bs_loc_ENU(2)], [ut_loc_ENU_single(3), bs_loc_ENU(3)], '--', 'Color', [0.85, 0.20, 0.20], 'LineWidth', 1.5, 'DisplayName', 'Line of Sight (UE 1)');

    % Ground Tangent Grid Wireframe
    grid_range = 25000.0;
    [x_g, y_g] = meshgrid(linspace(-grid_range, grid_range, 10), linspace(-grid_range, grid_range, 10));
    z_g = zeros(size(x_g));
    mesh(ax, x_g, y_g, z_g, 'EdgeColor', [0.1, 0.6, 0.2], 'FaceColor', 'none', 'EdgeAlpha', 0.25, 'HandleVisibility', 'off');

    % Velocity Vector Quivers
    scale_sat_vel = 5.0;
    quiver3(ax, bs_loc_ENU(1), bs_loc_ENU(2), bs_loc_ENU(3), v_sat_ENU(1)*scale_sat_vel, v_sat_ENU(2)*scale_sat_vel, v_sat_ENU(3)*scale_sat_vel, 0, 'Color', [0.0, 0.75, 0.75], 'LineWidth', 2, 'DisplayName', 'Sat Velocity (Scaled)');

    xlabel(ax, 'East (m)', 'FontSize', 12);
    ylabel(ax, 'North (m)', 'FontSize', 12);
    zlabel(ax, 'Up (m)', 'FontSize', 12);
    title(ax, sprintf(title_str), 'FontSize', 14, 'Color', 'k');
    legend(ax, 'Location', 'northeast', 'Color', 'w');
...
function plot_doppler_elevation_timeline(t_snapshot, set_elevation_deg, ...
    omega_s, u_mid, Omega_RAAN, inclination, r_orbit, v_sat_orbit, omega_E, ...
    a_wgs84, r_ue_ECEF_0, lambda, fc, c, pdf_filename)

    % Compute timeline from 10 deg horizon rise to 10 deg horizon set
    theta_horizon_rad = deg2rad(10.0);
    gamma_horizon = pi/2.0 - theta_horizon_rad - asin((a_wgs84 / r_orbit) * cos(theta_horizon_rad));
    t_pass_half = gamma_horizon / omega_s;

    t_grid = linspace(-t_pass_half, +t_pass_half, 200);
    elev_arr = zeros(1, length(t_grid));
    doppler_arr = zeros(1, length(t_grid));

    u_normal_bc = r_ue_ECEF_0 / norm(r_ue_ECEF_0);

    for k = 1:length(t_grid)
        tk = t_grid(k);
        [r_sat_k, v_sat_k] = get_satellite_state_ecef_local(tk, omega_s, u_mid, Omega_RAAN, inclination, r_orbit, v_sat_orbit, omega_E);
        v_los_k = r_sat_k - r_ue_ECEF_0;
        d_k = norm(v_los_k);
        u_los_k = v_los_k / d_k;
        
        elev_arr(k) = rad2deg(asin(dot(u_normal_bc, u_los_k)));
        doppler_arr(k) = (dot(v_sat_k, u_los_k) / c) * fc;
    end

    fig = figure('Visible', 'off', 'Color', 'w', 'Position', [100, 100, 850, 750]);

    % Subplot 1: Doppler Shift S-Curve
    subplot(2,1,1);
    plot(t_grid, doppler_arr / 1000, 'Color', [0.85, 0.20, 0.20], 'LineWidth', 2.0); hold on; grid on;
    xline(0, '--', 'Overhead Zenith (t=0s)', 'Color', [0.10, 0.65, 0.35], 'LineWidth', 1.5, 'LabelVerticalAlignment', 'bottom');
    xline(t_snapshot, ':', sprintf('Snapshot (t=%.1fs, %.0f°)', t_snapshot, set_elevation_deg), 'Color', [0.85, 0.60, 0.05], 'LineWidth', 1.8, 'LabelVerticalAlignment', 'top');
    xlabel('Time Relative to Overhead Zenith (seconds)', 'FontSize', 11);
    ylabel('Doppler Shift (kHz)', 'FontSize', 11);
    title('Satellite Doppler Shift Dynamics (S-Curve)', 'FontSize', 13);
    legend('Beam-Center Doppler Shift', 'Location', 'southwest', 'Color', 'w');

    % Subplot 2: Elevation Angle Dynamics
    subplot(2,1,2);
    plot(t_grid, elev_arr, 'Color', [0.0, 0.55, 0.65], 'LineWidth', 2.0); hold on; grid on;
    xline(0, '--', 'Overhead Zenith (t=0s)', 'Color', [0.10, 0.65, 0.35], 'LineWidth', 1.5, 'LabelVerticalAlignment', 'bottom');
    xline(t_snapshot, ':', sprintf('Snapshot (t=%.1fs, %.0f°)', t_snapshot, set_elevation_deg), 'Color', [0.85, 0.60, 0.05], 'LineWidth', 1.8, 'LabelVerticalAlignment', 'top');
    xlabel('Time Relative to Overhead Zenith (seconds)', 'FontSize', 11);
    ylabel('Elevation Angle (degrees)', 'FontSize', 11);
    title('Satellite Elevation Angle Dynamics', 'FontSize', 13);
    ylim([0, 95]);
    legend('Elevation Angle', 'Location', 'southwest', 'Color', 'w');
...
function [XS, YS, ZS] = get_local_earth_patch_local(lambda_ref, phi_ref, a_earth, delta_deg, num_pts)
    if nargin < 4, delta_deg = 12.0; end
    if nargin < 5, num_pts = 30; end

    lat_vec = linspace(phi_ref - deg2rad(delta_deg), phi_ref + deg2rad(delta_deg), num_pts);
    lon_vec = linspace(lambda_ref - deg2rad(delta_deg), lambda_ref + deg2rad(delta_deg), num_pts);
    [LAT, LON] = meshgrid(lat_vec, lon_vec);

    e2 = 6.69437999e-3;
    N_phi = a_earth ./ sqrt(1.0 - e2 * sin(LAT).^2);

    XS = N_phi .* cos(LAT) .* cos(LON);
    YS = N_phi .* cos(LAT) .* sin(LON);
    ZS = N_phi .* (1.0 - e2) .* sin(LAT);
end
